...
Go toolchain and `~/kvroute` there is not a git clone: it holds scp'd copies of `ops/`, the
cross-compiled `bin/*-linux-amd64`, and these scripts at its root. To use one, copy it to
`~/kvroute/` on the box and run it from there — every script sources `./lib-sweep.sh`
relative to that directory, and calls `./ops/fleet.sh` and `./bin/…` the same way.

They are committed as the exact files that produced a measurement, so a result can be traced
