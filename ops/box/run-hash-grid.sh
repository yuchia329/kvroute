...
# residency.
#
#   ./run-hash-grid.sh smoke      # one point: the window fits, the hash decides
#   ./run-hash-grid.sh weights    # the weight axis, 5 points, ~1.5 h
#   ./run-hash-grid.sh grid       # the grid arm at $WEIGHT, 12 points, ~3 h
#
...
# ops/versions.env -- and refuses to start on one that is not. Not because this
...
run_point() {
  ...
  say "$POLICY at ${BLOCKS}/${weight}: WS $ws skew $skew ($reps reps)"
  ./bin/bench-linux-amd64 \
    -router http://127.0.0.1:8080 \
    -dir "$base/ws$ws-skew$skew" \
    -policy "$POLICY" -hash "$BLOCKS/$weight" -fleet-kv-events=true \
    -concurrency "$CONC" \
