...
#
# Every input here has to be a committed measurement, because the point of the
# target is that a figure is rebuildable from a checkout. The 2026-09-16
# four-policy ladder is NOT one: ops/box/run-load-sweep.sh produces it, but its
# rows are gitignored and live only on the box that ran them, so pointing these
# variables at it draws figures nothing in the repository can redraw. Commit that
