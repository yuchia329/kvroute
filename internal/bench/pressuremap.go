...
type Validity struct {
	...
	// SpillHitRate and SpillLoad are the two spill conditions, counted over every
...
func (v Validity) Fired() bool {
	return v.RedundantPerRequest > 0 || v.HitRateSpread > 0 || v.SpillHitRate > 0 || v.SpillLoad > 0
}
...
func (g GridComparison) Validity(challenger string) Validity {
	...
	for _, mix := range g.Decisions {
		v.SpillHitRate += mix.SpillHitRate
