...
func writeCells(b *strings.Builder, scored []Rescored, everyCell bool) {
	...
	fmt.Fprintln(b)
	fmt.Fprintln(b, "| run | cell | rate | think | periods | recorded drift | current drift | basis | compared | confined | thin | backlog | recorded verdict | current verdict |")
	fmt.Fprintln(b, "|---|---|---:|---:|---:|---:|---:|---|---:|---:|---:|---:|---|---|")
	var changed int
	for _, r := range scored {
		if r.Changed() {
		...
		fmt.Fprintf(b, "| %s | %s | %s | %s | %s | %+.3f | %+.3f | %s | %d | %d | %d | %.0f%% | %s | %s |\n",
			r.Dir, r.Cell, rateOf(r), thinkOf(r), periodsOf(r),
			r.Was.WarmupDrift, r.Now.WarmupDrift,
			basisOf(r.Now), r.Now.WarmupDriftTurnsCompared, r.Now.WarmupDriftTurnsConfined, r.Now.WarmupDriftTurnsThin, r.Now.Backlog*100,
