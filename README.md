...
Five policies are implemented; four have been measured on the fleet. Every number below is
...
cmd/bench ──► router (:8080) ──► replica-0..5 (:8000..:8005, one GPU each)
    ...
    │             └─ five policies, exact per-replica inflight, scraped KV utilization,
...
- **`cmd/router`** — OpenAI-compatible `POST /v1/chat/completions`, SSE passed through untouched,
  ...
  leading run of the prompt's bytes and declines that match under load pressure; and
